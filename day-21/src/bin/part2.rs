...
fn part2(input: String) -> i64 {
    fn length(s: Vec<char>, depth: i32, dir_len: &HashMap<(char, char), usize>, seqs: &HashMap<(char, char), Vec<Vec<char>>>, cache: &mut HashMap<(Vec<char>, i32), i64>) -> i64 {
        if cache.contains_key(&(s.clone(), depth)) {
            return *cache.get(&(s.clone(), depth)).unwrap() as i64;
        }
        let mut len: i64 = 0;
        if depth == 1 {
            ...
            for (j, i) in s.iter().zip(&a_line) {
                len += *dir_len.get(&(*i, *j)).unwrap() as i64;
            }
        } else {
            let mut a_line = vec!['A'];
            a_line.extend(s.clone());
        ...
        cache.insert((s.clone(), depth), len);
        return len;
    }
    ...
    let mut cache: HashMap<(Vec<char>, i32), i64> = HashMap::new();
    ...
    for line in input.iter() {
        ...
        let mut m = std::i64::MAX;
        for p in poss.iter() {
            m = min(m, length(p.iter().copied().map(|c| *c).collect(), 25, &dir_len, &dir_seq, &mut cache));
        }
        complexity += m * (line.get(0..line.len() - 1).unwrap().parse::<i64>().unwrap());
    }
