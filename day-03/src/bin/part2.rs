...
fn part1(input: String) -> i32 {
    ...
    let mut if_do: bool = true; //part 2 requires an enabler

    // Loop through the input parts
    while index < input_parts.len() - 4 {
        // Check if the current part is a "mul(" substring or if mul() is enabled
        if &input_parts[index..index + 4].join("") != "mul(" || !if_do {
            //check if the next part changes the enabler
            if &input_parts[index..index + 4].join("") == "do()" {
                if_do = true;
                index += 4;
                continue;
            }
            if input_parts.len() > index + 6 && &input_parts[index..index + 7].join("") == "don't()" {
                if_do = false;
                index += 6;
                continue;
            }
